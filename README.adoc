...
. Wrap your clean build command with the Build Wrapper. E.g.: `build-wrapper-win-x86-64.exe --out-dir bw-output MSBuild.exe /t:Rebuild` (with `bw-output` being the same directory that the one set in `sonar-project.properties`)
. Run `sonar-scanner`
